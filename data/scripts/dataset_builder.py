...
def add_tyre_change_pursuer(df: pd.DataFrame) -> pd.DataFrame:
    ...
    df["did_change_since_prev"] = (df["compound_norm"].notna() & prev_comp.notna() & (df["compound_norm"] != prev_comp))
...
def make_dataset_1(df_full: pd.DataFrame) -> pd.DataFrame:
    """
    Dataset 1 (clean DB):
      - Same as your current output, WITHOUT y_compound
      - Adds fulfilled_second_compound
    """
    keep_cols = [
...
def make_dataset_2(df_full: pd.DataFrame) -> pd.DataFrame:
    ...
    Dataset 2 (less-clean DB):
      ...
      - Only the requested attributes (do NOT include y_pit)
      - PLUS race_id to allow race-wise train/test splitting (avoid temporal leakage)
    """
    df_pit = df_full[df_full.get("y_pit", 0).astype(int) == 1].copy()

    keep_cols = [
        # NEW: race identifier for group splits
        "race_id",
