...
def delete_by_pairs(conn, pairs):
    ...
    #creates a table with all driver race combinations we want to remove and removes them all at once
...
def remove_late_pitstops(conn):
    # removes pit stops in the last 10% of a dry race (usually weird outliers) only for 2019-2024 seasons
    sql = """
    DELETE FROM laps
    WHERE rowid IN (
        SELECT l.rowid
...
def remove_by_position(conn, max_pos):
    #removes all drivers who finish outside the top max_pos (10 or 15 in this case)
    sql = """
...
def remove_red_flags(conn):
    # gets rid of laps where the red flag was out (track status code 5)
    before = conn.total_changes
...
def run_less_clean(conn):
    ...
    #gets rid of drivers races where the race was dry and the driver did over 3 pit stops
    ...
    #gets rid of drivers who finished > 15th
